<?php
namespace Roots\Sage\SaveImageFromRemote;
use WP_Error;
 ...
 *
 *
 * @file           save-image-from-remote-to-post.php
 * @package        Touko Aalto 2017
 * @author         Janne Saarela
 * @version        0.1.0
 * @filesource     wp-content/themes/toukoaalto2017/lib/save-image-from-remote-to-post.php
...
add_filter( 'save_image_from_remote_to_post', __NAMESPACE__ . '\\save_image_from_remote_to_post', 1, 2 );
...
function save_image_from_remote_to_post( $url = null, $post_id = null ) {
    ...
    if ( ! empty( $url ) && ! empty( $post_id ) ) :
		...
		try {
			...
			return new WP_Error( 'image-download-error', __( 'Image download error', THEME_SLUG ) );
		}
		...
		if ( sizeof( $matches ) > 0 ) :
			...
			if ( filter_var( $upload_dir['baseurl'], FILTER_VALIDATE_URL ) === false ) {
                ...
                $file_url = $upload_dir['baseurl'] . '/' . TEMP_IMAGE_FOLDER . '/' . $file_name;
			}
            ...
            if ( $new_file ) :
				...
				if ( is_wp_error( $tmp ) ) {
				    ...
				    return new WP_Error( 'image-write-error', __( 'Error while writing new file', THEME_SLUG ) );

			endif;

		else :

			return new WP_Error( 'image-content-type-error', __( 'Image type is not permitted', THEME_SLUG ) );

		endif;

  else :

		return new WP_Error( 'image-url-id-error', __( 'Image url or post id can not be empty!', THEME_SLUG ) );
